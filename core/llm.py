...
双后端（按 key 自动选，调用方零改动）：
  1. ANTHROPIC_API_KEY 存在 → 官方 Anthropic Messages API（用户自己的 key，自担费用）。
     模型默认 claude-sonnet-5（2026-07-08 升级，thinking 显式关闭保行为可控，
     详见下方常量注释），可用 ANTHROPIC_MODEL / ANTHROPIC_THINKING 环境变量覆盖。
...
# 模型（2026-07-08 定）：claude-sonnet-5 —— Sonnet 系最新代，指令遵循/推理显著强于 4.5
# （对本项目"prompt 铁律+守卫+严格 JSON"架构是正向红利），且推广价 $2/$10（至 2026-08-31）
# 比 sonnet-4-5（$3/$15）还便宜。Opus 4.8（$5/$25）对短结构化判断性价比不足，Fable 更贵，不用。
ANTHROPIC_MODEL = os.environ.get("ANTHROPIC_MODEL", "claude-sonnet-5")
# 🔴 thinking 坑：Sonnet 5 不传 thinking 时**默认开自适应思考**，思考 token 挤占 max_tokens
# （本项目最大才 2000）→ JSON 输出可能被截断炸解析器/守卫。默认显式关掉 = 行为与 sonnet-4.5
# 等价、模型底子更强。想实验思考：ANTHROPIC_THINKING=adaptive（记得同时加大调用点 max_tokens）；
# ANTHROPIC_THINKING=omit 则不发该参数（换老模型如 sonnet-4-5 时用，老模型不认 disabled）。
ANTHROPIC_THINKING = os.environ.get("ANTHROPIC_THINKING", "disabled")
...
def _anthropic_body(prompt: str, max_tokens: int) -> dict:
    body = {
        "model": ANTHROPIC_MODEL,
        "max_tokens": max_tokens,
        "messages": [{"role": "user", "content": prompt}],
    }
    if ANTHROPIC_THINKING in ("disabled", "adaptive"):
        body["thinking"] = {"type": ANTHROPIC_THINKING}
    return body
...
def _call_anthropic(key: str, prompt: str, max_tokens: int, timeout: int) -> str:
    try:
        resp = requests.post(
            ...
            json=_anthropic_body(prompt, max_tokens),
            timeout=timeout,
        )
