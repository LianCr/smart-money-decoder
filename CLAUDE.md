...
**smart-money-decoder**：输入 Polymarket 交易者钱包地址 → 找出其最大政治预测盘仓位 → 结合实时新闻生成 AI 解读卡片（在赌什么 / 催化剂 / 还值不值得跟）。只读 API，不涉及交易或私钥。
...
api/main.py             →  GET /analyze（v2 解读卡）· GET /backtest（静态）· GET /briefing（v3 完整简报，钱包→顶仓→assemble+organize，按(钱包,AS_OF)整份硬缓存零 token）
...
frontend/ (Vite+React)  →  src/App.jsx 单页（Decode 实时 / Track Record / Briefing 完整简报）· src/index.css
...
**置信度矩阵**（代码算，优先级高→低）：articles 空→低(强制) · pnl_pct>60%→低 · pnl_pct<0% 且(无新闻或未锚定)→低 · pnl_pct<0%→中(封顶) · time_anchored=False→中(封顶) · 有新闻+anchored+0≤pnl<30%→高 · 有新闻+anchored+30≤pnl<60%→中
...
## 🟢 v3 方向（待用户定节奏才启动，不擅自开工）

完整蓝图在 `KNOWN_ISSUES.md`（第七类世界观 / 第八类 v3 愿景 / 第九类数据 API / 第十类导师反馈）。指针：

- **v3 第一仗 = 路 B「离场盈亏」口径**：performance 从"测判断方向"升级为"假设 $1000 跟入 AI 说 GO 的注、算真实平均收益率 vs 无脑全抄基线"。依赖新数据 API 的真实 PnL。
- **数据地基 = `api.polymarketanalytics.com`（heisenberg）**：`556 Trades`+`569 PnL`（解锁离场盈亏）· `584 Falcon Score`+`581 Wallet 360`（可信度引擎/信心增强）· `585 Social Pulse`（换掉弱新闻源）· `Kalshi`/`cross`（跨平台 alpha）。**启动前先查价格/免费额度 + 验证数据质量**。
- **护城河**：不在数据（谁都能买），在判断（聪明钱行为 + 新闻 + 价格三合一的可信判断）。这个 API 越强越说明拼的是"解读得准、解读得诚实"。
