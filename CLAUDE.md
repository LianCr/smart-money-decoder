...
7. **代码守最佳实践：单一职责、模块化，别堆积。** 🔴 反面教材（欠着的技术债）：`frontend/src/App.jsx` **2210 行塞 ~35 个组件**、`index.css` 1260 行、**无 `components/` 拆分**。**新增/重构 UI 一律抽到 `frontend/src/components/`·`views/`·`hooks/`，禁止继续往 App.jsx / index.css 堆。** 后端同理——每个模块一摊事，跨了职责就拆。
